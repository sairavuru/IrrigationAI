...
def calcEvi(red_array,nir_array) -> "numpy array":
	"""
...
def toRaster(in_array,out_path,model_file,dtype = None) -> None:
	...
	dataset = driver.Create(out_path,rasterXSize,rasterYSize,1,outType,['BIGTIFF=YES', 'COMPRESS=DEFLATE'])
...
def toRasterAll(in_array,out_path,model_file,dtype = None) -> None:
	"""
	This function saves a numpy array into a raster file, with
	the same project and extents as the provided model file.
	...
	if geoTransform[1] == 1.0:
		if os.path.splitext(model_file)[1] == ".h5":
			...
			geoTransform = ds_sub.GetGeoTransform()
			sr = ds_sub.GetProjection()
	rasterXSize = int(in_array[0,:,:].shape[0])
	rasterYSize = int(in_array[0,:,:].shape[1])
	refDs = None
	...
	## write to disk
	driver = gdal.GetDriverByName('GTiff')
	dataset = driver.Create(out_path,rasterXSize,rasterYSize,nband,outType,['BIGTIFF=YES', 'COMPRESS=DEFLATE'])

	for band in range(nband):
		print('Writing band # ', str(band))
		rast_arr = in_array[band,:,:]
		dataset.GetRasterBand(band+1).WriteArray(rast_arr.astype(np.int16))
		dataset.GetRasterBand(band+1).SetNoDataValue(-3000)
		dataset.SetGeoTransform(geoTransform)
	dataset.FlushCache() # Write to disk
	del dataset
